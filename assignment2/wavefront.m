...
for i = 1:N
    for j = 1:M
        ...
        if (map(i,j) ~= 0)
            if((i-2 > 0) && (j-2 > 0))
                paddedMap(i-2:i,j-2:j) = map(i,j);
            elseif((i-2 < 1) && (j-2 > 1))
                paddedMap(1:i,j-2:j) = map(i,j);
            elseif((i-2 > 1) && (j-2 < 1))
                paddedMap(i-2:i,1:j) = map(i,j);
            elseif((i-2 < 1) && (j-2 < 1))
                paddedMap(1:i,1:j) = map(i,j);
            end
            
            if((i+2 < N+1) && (j-2 > 0))
                paddedMap(i:i+2,j-2:j) = map(i,j);
            end
            if((i-2 > 0) && (j+2 < M+1))
                paddedMap(i-2:i,j:j+2) = map(i,j);
            end
            
            if((i+2 < N+1) && (j+2 < M+1))
                paddedMap(i:i+2,j:j+2) = map(i,j);
            elseif((i+2 > N+1) && (j+2 < M+1))
                paddedMap(i:N,j:j+2) = map(i,j);
            elseif((i+2 < N+1) && (j+2 > M+1))
                paddedMap(i:i+2,j:M) = map(i,j);
            elseif((i+2 > N+1) && (j+2 > M+1))
